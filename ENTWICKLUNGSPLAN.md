...
| **Der Sensor-Takt hängt am langsamsten Listener** — `fire()` läuft synchron auf dem Sampler-Thread, ein langsamer Listener verlängert also die Runde und die nächste Messung folgt entsprechend später | Ein *Rückstau* ist das ausdrücklich nicht: `scheduleWithFixedDelay` beginnt die Pause erst nach dem Ende der Runde, es läuft also nichts auf — die Messung wird nur seltener. Auflaufen könnte erst etwas bei echter Hardware, die unabhängig weitermisst; dann bräuchte es eine Entkopplung über eine Queue. Die einzige Stelle, an der hier tatsächlich etwas wachsen kann, ist die unbegrenzte Warteschlange von `Platform.runLater` — bei 500 ms Takt kommt der FX-Thread mühelos mit. Beides zu lösen wäre Aufwand ohne Erkenntnisgewinn. |
...
| 04.09.2026 | 2 | Befund von Patrick beim Gegenlesen des READMEs: Die Begründung für `CopyOnWriteArrayList` betonte die falsche Größe. Dort stand „bei einer Handvoll Listenern belanglos, bei Tausenden die falsche Wahl" — also die **Länge** der Liste als ausschlaggebend. | **Übernommen.** Kopiert wird ausschließlich beim Ändern der Liste (`subscribe` in Zeile 26, `close` in Zeile 57); `fire` iteriert nur und kopiert nie. Ausschlaggebend ist damit die **Häufigkeit der Änderungen**, die Länge multipliziert nur den Preis einer einzelnen Kopie. Tausend Listener, die sich einmal anmelden, sind unproblematisch; fünf, die sich hundertmal pro Sekunde an- und abmelden, wären es nicht. In dieser Anwendung fallen zwei Kopien im ganzen Programmlauf an. Korrigiert in beiden READMEs und in `CLAUDE.md`. |
| 04.09.2026 | alle | Patrick äußerte nach dem `CopyOnWriteArrayList`-Fund die Sorge, wie viele weitere Aussagen in den Dokumenten falsch sein könnten. Daraufhin systematische Gegenprüfung: 22 nachprüfbare Behauptungen aus beiden READMEs, `CLAUDE.md` und diesem Plan gegen den Code gehalten. | **20 bestätigt** (privates Schloss auch bei lesenden Zugriffen, `fire()` außerhalb des Sperrbereichs, Ereignis innerhalb gebaut, Startwert = Sollwert, Bereichsprüfung im Gerät, alle Konstanten, `catch` innerhalb der Listener-Schleife, Taktrate, Build-Versionen, Abhängigkeitsrichtung mechanisch per Suche nach `incubator.ui` in `device`/`events`/`src/test`). **Zwei Funde, beide korrigiert:** (1) „die Laufzeit ruft `Application.stop()` auf jedem Weg des Herunterfahrens auf" — falsch, `System.exit()` umgeht es; wieder ein unnötiges Absolutum. (2) Die Erklärung, der Jungbereich eines kleineren Heaps fülle sich schneller, war als Tatsache formuliert, ist aber eine Deutung der Zahlen — jetzt als solche gekennzeichnet. |
...
| 04.09.2026 | 3 | Review der `start()`-Methode des `TemperatureSampler` auf Anfrage, mit enger Frage: redundante lokale Variablen oder fehlende Werte? | **Ein Befund von zweien übernommen.** *Nicht übernommen:* Die benannten `ThreadFactory`- und `Runnable`-Variablen sind nicht redundant — sie ließen sich in die Aufrufe hineinziehen, aber die Namen tragen die Lesbarkeit der Methode. *Übernommen:* Das Intervall wurde nur mit `Objects.requireNonNull` geprüft, nie auf ein **positives** Vorzeichen. `Duration.ZERO` kam damit durch den Konstruktor und scheiterte erst in `start()`, wo `scheduleWithFixedDelay` eine Wartezeit von null mit `IllegalArgumentException` ablehnt — der Fehler wäre also weit entfernt von seiner Ursache aufgeschlagen. Dazu schnitt `interval.toMillis()` ab: `Duration.ofNanos(500_000)` wurde zu `0` und scheiterte an derselben Stelle, obwohl der übergebene Wert gültig war. Reparatur: `requireValidInterval` im Konstruktor (gleicher Zuschnitt wie die beiden Prüfmethoden im `Incubator`), Zeitangabe auf `toNanos()` mit `TimeUnit.NANOSECONDS`. Zwei neue Tests, beide vorher rot gesehen: `anIntervalOfZeroOrLessIsRejected` (keine Ausnahme kam) und `anIntervalBelowOneMillisecondSurvives` (Ausnahme kam, aber aus `start()` statt gar nicht). 24 Tests grün. Ein dritter, rein kosmetischer Hinweis — zwei `else`-Zweige hinter einem `return` in `start()` und `stop()` — wurde von Patrick selbst erledigt, bevor der Rest angefasst wurde. Beide READMEs haben dafür einen Abschnitt „Wo KI beteiligt war" bekommen. |

| 07.09.2026 | 3 | Beim Vorbereiten von Interview-Antworten fragte Patrick, warum es überhaupt einen Rückstau gebe, obwohl der Sampler `scheduleWithFixedDelay` nutzt. Die Frage deckte einen Fehler in der eigenen Formulierung auf. | **Übernommen.** „Kein Rückstau-Schutz" war die falsche Bezeichnung: `scheduleWithFixedDelay` beginnt die Pause erst nach dem Ende der Runde, es kann also nichts auflaufen. Der tatsächliche Effekt ist **Kopplung** — `fire()` läuft synchron auf dem Sampler-Thread, ein langsamer Listener verlängert die Runde, und die Messung wird dadurch seltener statt verspätet nachgeholt. Echter Rückstau entstünde erst bei Hardware, die unabhängig weitermisst. Die einzige Stelle in diesem Programm, an der tatsächlich etwas wachsen kann, ist die unbegrenzte Warteschlange von `Platform.runLater`; bei 500 ms Takt folgenlos. Umformuliert in diesem Plan sowie in `CLAUDE.md`, `README.de.md` und `README.md` (dort hieß es „No backpressure"); die drei kürzeren Fassungen nennen den `runLater`-Zusatz nicht, das Detail trägt dieser Plan. Nebenbei aufgefallen: In derselben Antwort waren mehrere Zeilenverweise falsch, weil beim Lesen mehrerer Dateien in einem Rutsch der Versatz der durchlaufenden Nummerierung nicht abgezogen wurde. Merksatz: Zeilenangaben nur aus einer Einzeldatei-Ausgabe übernehmen. |
| 07.09.2026 | 5 | Review des Abschnitts „Wo KI beteiligt war" in beiden READMEs auf Anfrage. Fünf Befunde: (1) Der Einstiegssatz „Claude Code war Reviewer, nicht Code-Generator" widerspricht der Regel zwei Zeilen darunter — „Generierter Code wird nicht übernommen, bevor er verstanden ist" setzt voraus, dass generiert wird. (2) Der Satz ist ein Absolutum und von außen widerlegbar: Diese Dokumente sind sichtbar im Dialog entstanden, was das Protokoll hier selbst festhält. Dieselbe Fehlerklasse wie am 04.09. zweimal zuvor. (3) Der Abschnitt zeigt nur die Review-Form, obwohl der Plan vier Formen der Zusammenarbeit kennt und Form 2 („Entwurf vorab durchsprechen") dort als die wertvollste bezeichnet wird. (4) „Die jüngste Runde" veraltet — durch den Eintrag darüber war sie es schon nicht mehr. (5) „Zwei Antworten kamen zurück", während weiter unten ein dritter, kosmetischer Befund auftaucht. | **Übernommen, in beiden READMEs.** Der Einstieg benennt jetzt offen, wo KI beteiligt war (Reviews, Entwürfe vorab, diese Dokumente) und grenzt ab, was nicht: der Code ist von Hand getippt. Der tragende Satz — „Die Befunde kamen aus dem Review; die Entscheidung, welcher davon eine Änderung wert war, nicht" — steht statt des Absolutums oben, weil er belegbar ist und vorher am Ende stand. „Die jüngste Runde" ersatzlos gestrichen, damit der Abschnitt beim nächsten Protokolleintrag nicht veraltet. Die drei Befunde stehen als drei Punkte mit dem Urteil im Titel (*nicht übernommen* / *übernommen* / *kosmetisch*) statt zwei Punkten plus Nachsatz — die Trennung, um die es dem Abschnitt geht, war vorher aus dem Fließtext zu holen. **Nicht übernommen:** der Vorschlag, den Beispielfall vom `TemperatureSampler` auf den Rückstau-Fall des Eintrags darüber zu wechseln — dort korrigiert die Rückfrage zwar das Werkzeug statt umgekehrt, aber der `TemperatureSampler`-Fall zeigt drei unterschiedlich behandelte Befunde aus einer Anfrage, und genau das ist der Punkt des Abschnitts. Zurückgestellt, nicht verworfen. |
...
| 04.09.2026 | Anspruch im README von „jede Entscheidung im Code begründen können" auf „nachvollziehbar zu sein" zurückgenommen; die tragenden Entwurfsentscheidungen dafür ausformuliert | Einwand von Patrick: Ein Absolutum in einem öffentlichen README ist eine Behauptung, die ein Leser an einer Stelle seiner Wahl prüfen darf — dann geht es nicht mehr um die Qualität der Entscheidungen, sondern um die Haltbarkeit des Satzes. Der neue Satz verweist auf Belege, die es tatsächlich gibt (Änderungsprotokoll, Review-Protokoll, offene Punkte), statt eine Eigenschaft des Autors zu behaupten. Im selben Zug wurde der Abschnitt geteilt: „Warum der Code so aussieht" (privates Schloss, `fire()` außerhalb, zustandsloser Sampler, `scheduleWithFixedDelay`, Daemon-Threads, Lesen vor `runLater`, Schnitt der View, `Application.stop()`, `CopyOnWriteArrayList`) und „Werkzeugwahl". Vorher enthielt der Abschnitt fast nur Werkzeugwahl — die Entscheidungen, die den Code tragen, standen nur als Fließtext bei den Etappen. |
