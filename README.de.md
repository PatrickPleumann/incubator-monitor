...
Claude Code war durchgehend beteiligt — bei Reviews, beim Durchsprechen von Entwürfen vor dem
Tippen und an diesen Dokumenten. Der Code ist von Hand getippt, unter einer Regel, die vor dem
ersten Commit feststand: *Generierter Code wird nicht übernommen, bevor er verstanden ist.* Die
Befunde kamen aus dem Review; die Entscheidung, welcher davon eine Änderung wert war, nicht.

Wie das aussieht — `TemperatureSampler`, mit einer bewusst engen Frage übergeben: **Gibt es
redundante lokale Variablen oder fehlende Werte?** Drei Anmerkungen kamen zurück, jede wurde
anders behandelt.

- **Keine redundanten lokalen Variablen — nicht übernommen.** Die benannte `ThreadFactory` und das
  `Runnable` in `start()` ließen sich in ihre Aufrufe hineinziehen. Redundant sind sie deshalb
  nicht — die Namen sind es, die die Methode lesbar halten. Hier wurde nichts geändert.
- **Ein fehlender Wert — übernommen.** Das Messintervall wurde auf `null` geprüft, aber nie
  darauf, ob es **positiv** ist. `Duration.ZERO` kam durch den Konstruktor und scheiterte erst
  später in `start()`, wo `scheduleWithFixedDelay` eine Wartezeit von null ablehnt — eine Ausnahme
  weit weg von ihrer Ursache. Daneben schnitt `interval.toMillis()` ab: Ein Intervall unter einer
  Millisekunde wurde zu null und scheiterte genauso.
- **Zwei `else`-Zweige hinter einem `return` — kosmetisch.** Von Hand erledigt, bevor der Rest
  angefasst wurde.
...
Alle drei stehen im Review-Protokoll von [`ENTWICKLUNGSPLAN.md`](ENTWICKLUNGSPLAN.md), zusammen mit
jedem anderen Befund, der nicht übernommen wurde.
