...
| **`CopyOnWriteArrayList` im EventSupport** | Sichere Iteration, während sich ein Listener abmeldet. Kopiert wird nur beim Ändern der Liste — `subscribe` und `close`, nicht `fire`. Hier also zwei Kopien im ganzen Programmlauf. Entscheidend ist die Häufigkeit der Änderungen, nicht die Länge der Liste; letztere multipliziert nur den Preis einer einzelnen Kopie. |
